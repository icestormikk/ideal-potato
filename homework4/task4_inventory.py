maxWeight = 90
inventory = [
    ["Witcher's steel sword", 1, 1.74], 
    ["Wolf School Medallion", 1, 0.5], 
    ["Swallow", 10, 0.1], 
    ["A letter (with the scent of lilac and gooseberries)", 1, 0.0],
    ["Hunting gauntlets", 1, 0.67], 
    ["Kaer Morhen armor", 1, 1.57], 
    ["Hunting trousers", 1, 1.06], 
    ["Hunting boots", 1, 0.71]
]

def duplicateItem(name, weigth=None):
    """Проверка на содержание в инвентаре предметов с тем же именем
    и весом(если передан). 
    
    Аргументы:\n
    name - название предмета;\n
    weigth - вес предмета (по умолчанию None)
    """
...
while True:
    ...
    if (user_in == "additem"):
        ...
        if (len(inventory) > 0):
            maxLength = 0;
            #расчёт наибольшей длины строки с названием предмета
            for i in inventory:
                if (len(i[0]) > maxLength):
                    maxLength = len(i[0])

            #вывод содержимого инвентаря в виде таблицы со столбцами:
            #"Название предмета"  "Кол-во копий"  "Общий вес"(вес 1-ой единицы * кол-во)
